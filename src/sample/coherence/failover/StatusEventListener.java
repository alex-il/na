...
public class StatusEventListener implements MapListener{
	...
	private void processEvent(MapEvent mapEvent){
		...
		executer.execute(new Runnable() {
			...
			public void run() {
				try {
					String memberName = ManagementFactory
							.getRuntimeMXBean().getName();
					Logger.log(Logger.DEBUG, memberName + " : EventListener: Thread:" + Thread.currentThread().getId() + ":" +  MapEvent.getDescription(oper) + " MsgId=" + event.toString() );
					NamedCache cache = CacheFactory
							.getCache(StatusEvent.EVENTS_CACHE);
